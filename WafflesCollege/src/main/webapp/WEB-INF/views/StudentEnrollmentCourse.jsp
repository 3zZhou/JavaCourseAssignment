	...
	<section>
		<div class="col-container">
			<nav>
				...
				<a href="${pageContext.request.contextPath}/student/enrolnew" class="sidebar-button"><b>Enroll into a new Course</b></a> 
				...
				<c:if test="${fn:length(Display) gt 0}">
					...
					<table class="borderAll">
						<tr>
							...
							<th><s:message text="&nbspEnrol&nbsp"/></th>
						</tr>
						<!-- cseID, cseDesc, cseLecturer, cseCredits, cseStatus -->
						<c:forEach var="course" items="${Display}" varStatus="status">
							<tr class="${status.index%2==0?'even':'odd'}">
								<td style="text-align: center;" class="nowrap">${status.index+1}</td>
								<td style="text-align: center;" class="nowrap">${course.cseId}</td>
								<td style="text-align: center;" class="nowrap">${course.cseDesc}</td>
								<td style="text-align: center;" class="nowrap">${course.cseCredit}</td>
								<td style="text-align: center;" class="nowrap">${course.lecturer.lecFirstmidname}${course.lecturer.lecLastname}</td>
								
								<td><a href="${pageContext.request.contextPath}/student/enrolnewform/${course.cseId}">Enrol</a>
								</td>
